...
std::complex<double> Heston::characteristic_function(std::complex<double> u) const {
    ...
    std::complex<double> C =
        option.free_risk_interest * i * u * option.maturity +
...
double Heston::price_call_impl() const {
    ...
    return option.Spot * exp(-option.devidende * option.maturity) * P1 -
           option.strike * exp(-option.free_risk_interest * option.maturity) * P2;
}

double Heston::price_put_impl() const {
    const double P1 = 0.5 + (1.0 / M_PI) * integrate(1);
    const double P2 = 0.5 + (1.0 / M_PI) * integrate(2);
    return option.strike * exp(-option.free_risk_interest * option.maturity) * (1.0 - P2) -
           option.Spot * exp(-option.devidende * option.maturity) * (1.0 - P1);
}
